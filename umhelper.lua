...
local id_name = '##UM Helper'
local tag = '{FFA500}[UM Helper]: '
local tmp = {['downKey'] = {}}
...
function main()
	...
	RegisterCallback('repcar', setup.keys.repcar, repcar)
	...
	sampAddChatMessage(tag .. u8:decode('{FFFFFF}Скрипт успешно загружен. Автор: {FFA500}Vadim_Kiselev.'), 0x1E90FF)
	...
	hotkey.List["repcar"] = {
    keys = {} -- Пустая последовательность клавиш
	}	
	...
	function(player)
		...
		imgui.Begin('News Helper by Kiselev ##window_1', rMain, imgui.WindowFlags.NoCollapse + (not cheBoxSize[0] and imgui.WindowFlags.NoResize or 0) + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoScrollWithMouse) -- + imgui.WindowFlags.NoMove + imgui.WindowFlags.AlwaysAutoResize
			...
			imgui.BeginChild(id_name .. 'child_window_1', imgui.ImVec2(imgui.GetWindowWidth() - 6, 30), false)
				...
				imgui.TextStart('UM Helper by Kiselev')
				imgui.NextColumn()
				imgui.TextCenter('v'..thisScript().version..' beta')
				imgui.NextColumn()
				imgui.TextEnd('Promo: #kiselevlove')
			...
			imgui.BeginChild(id_name .. 'child_window_3', imgui.ImVec2(imgui.GetWindowWidth() - 154, imgui.GetWindowHeight() - 47), true, imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoScrollWithMouse)
				...
				elseif mainPages[0] == 2 then imgui.pravilavc()
				elseif mainPages[0] == 3 then imgui.pravilastrel()
				elseif mainPages[0] == 4 then imgui.pravilaporta()
...
imgui.OnFrame(function() return tst[0] end,
	function(player)
			imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
			imgui.SetNextWindowSizeConstraints(imgui.ImVec2(700, 400), imgui.ImVec2(800, 500))
			imgui.Begin("test")
			imgui.Link('https://vk.com/ssker', u8'Написать Разработчику')
			imgui.End()
	end
)
	...
	function(player)
		...
		imgui.Begin('Меню быстрого доступа ##window_4', rFastM, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollWithMouse + imgui.WindowFlags.NoScrollbar --[[+ imgui.WindowFlags.NoTitleBar]]) -- + imgui.WindowFlags.AlwaysAutoResize imgui.TabBarFlags.NoCloseWithMiddleMouseButton
			...
			imgui.BeginChild(id_name .. 'child_window_7', imgui.ImVec2(imgui.GetWindowWidth() - ((imgui.GetWindowWidth() - wPaddX*2) / 1.7) - 2 - wPaddX, imgui.GetWindowHeight() - 2 - wPaddY*2), true)
				...
				imgui.CustomMenu({'Собеседование'}, fastPages, imgui.ImVec2(120, 35), 0.08, true, 15)
			imgui.EndChild()
		imgui.End()
		imgui.SetMouseCursor(-1)
	end
...
function imgui.WindowMain() -- Основное окно
	...
	imgui.BeginChild(id_name..'child_7', imgui.ImVec2(imgui.GetWindowWidth() - 195, 180), false, 0)
		imgui.TextWrapped('Скрипт помощник для Украинской Мафии. Сделан по многочисленным просьбам, для состава Kardoni. Скрипт нацелен именно на помощь, а не автоматизацию. Функции "Бота" тут отсутствуют, скрипт стремится к легализации.\nНа данный момент скрипт находится в альфа версии - все кнопки, интерфейс, система могут или будут переделаны, сейчас скрипт распространяется для сбора критических ошибок и предложений по улучшению скрипта.')
	imgui.EndChild()
	imgui.BeginChild(id_name..'child_8', imgui.ImVec2(imgui.GetWindowWidth() - 13, imgui.GetWindowHeight() - 202), false, 0)
		...
		imgui.Link('https://vk.com/ssker', 'Написать Разработчику', imgui.ImVec2(150, 22))
...
function imgui.ScrSettings() -- Настройки
	...
	if KeyEditor('repcar', '/repcar', imgui.ImVec2(280,25)) then
		saveKeysBind()
	end
end
...
function imgui.FmInterviews()
	local refusals = {
		...
		{'Нет 3 ур', function ()
			sampSendChat(u8:decode('Извините но чтобы работать нужно иметь 3-х летнюю прописку в округе.'))
			wait(1000)
	...
	local buttons = {
		...
		{'Запрос документов', function ()
			sampSendChat(u8:decode('Хорошо, покажите ваши документы. А именно паспорт, лицензии и выписку из тира.'))
			wait(1000)
			local myId = select(2,sampGetPlayerIdByCharHandle(PLAYER_PED))
			sampSendChat(u8:decode(string.format('/b /showpass %s | /showlic %s | /showskill %s', myId, myId, myId)))
		end},
...
function imgui.pravilavc() -- Подраздел Собседование
	imgui.BeginChild(id_name..'child_7', imgui.ImVec2(imgui.GetWindowWidth() - 30, 300), false, 0)
		imgui.TextWrapped('\t\t\t\t\t\t\t\t\t\t\t\t\tПравила Нападения на ВЧ\n\n1.1 Нападать на военную часть могут только мафии/ОПГ/участники семей по одному разу в день от всего/всей мафии/ОПГ/семьи. Любое попадание на военную часть игроков не соблюдая правила нападения на военную часть карается деморганом.\n\n1.2 Нападения на Военную часть разрешены:\n\nВ будние дни с 16:00 до 21:00.\nВ выходные дни с 15:00 до 20:00.\n\n2.1 Мафии, либо ОПГ должна устроиться работником мусоровоза , после чего арендовать мусоровоз (/gps - Работа - Сстоянка мусоровозов) и далее действовать по одному из двух планов:\n\nТихо заехать на нем на военную часть через главное КПП, набрать ингредиентов у завода и тихо уехать.\nВзять в плен армейца, заставить его загрузить военные уралы и тихо уехать.\n\n')
	imgui.EndChild()
end
function imgui.pravilastrel() -- Подраздел Собседование
	imgui.BeginChild(id_name..'child_7', imgui.ImVec2(imgui.GetWindowWidth() - 30, 300), false, 0)
		imgui.TextWrapped('\t\t\t\t\t\t\t\t\t\t\t\t\tПравила стрел')
	imgui.EndChild()
end
function imgui.pravilaporta() -- Подраздел Собседование
	imgui.BeginChild(id_name..'child_7', imgui.ImVec2(imgui.GetWindowWidth() - 30, 300), false, 0)
		imgui.TextWrapped('\t\t\t\t\t\t\t\t\t\t\t\t\tПравила Нападения на порт\n\n Время прибытия военного корабля:\n\n11:45\n14:45\n16:45\n18:45\n20:45')
	imgui.EndChild()
end
function imgui.EventsSetting() -- раздел эфир. Настройки
	imgui.BeginChild(id_name..'child_window_13', imgui.ImVec2(imgui.GetWindowWidth() - 12, imgui.GetWindowHeight() - 40), false)
		...
		if (msg == 0x0100 or msg == 260) and not sampIsChatInputActive() then --and not sampIsDialogActive()
			if hotkey.EditKey == nil then
				...
				elseif key == vk.VK_BACK then
					hotkey.List[hotkey.EditKey].keys = {VK.Q}
					hotkey.EditKey = nil
					saveKeysBind()
					clearButtons()
...
function Style()
    ...
    colors[clr.ButtonHovered] = ImVec4(0.11, 0.56, 0.06, 1.00)
    colors[clr.ButtonActive] = ImVec4(0.24, 0.81, 0.003, 1.00)
